...
## write a function to save some space
...
## get tables
spring.19.q1 <- spring.19 %>% slice(1) %>% select(-c(X4:X7)) %>% rename(Responses = "N.answers")
...
```{r cache=FALSE, collapse=TRUE, echo=FALSE}
knitr::kable(
  spring.19.q1, 
  caption = "Spring 2019 Student Evaluations, Statistics Lab",
  align = "lccc") %>% 
...
<button 
onclick="location.href='https://matthewevanaman.netlify.app//PDFs/eval_fall18.pdf'"
type="button" target='_blank' >View Faculty Evaluation
</button>
...
knitr::kable(
  fall.18.q1, 
  caption = "Fall 2018 Student Evaluations, Statistics Lab",
   align = "lccc") %>% 
  kableExtra::kable_styling(position = "left")
